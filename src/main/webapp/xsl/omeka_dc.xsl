...
<xsl:stylesheet version="2.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	xmlns:mods="http://www.loc.gov/mods/v3" exclude-result-prefixes="mods srw_dc cdwalite lcloud"
...
-->
	...
	<xsl:template match="lcloud:pagination">
		<xsl:call-template name="pagination">
			<xsl:with-param name="metadataPrefix" select="'omeka_dc'"/>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template match="mods:mods"> 
		<xsl:element name="record">
			<xsl:element name="header">
				...
				<xsl:element name="setSpec"></xsl:element>
			</xsl:element>
	...
	<xsl:template match="mods:titleInfo">
		<dc:title>
			...
			<xsl:if test="../mods:relatedItem[@type='host']">
				<xsl:text> (</xsl:text><xsl:value-of select="string-join(reverse(../descendant::mods:relatedItem/mods:titleInfo/mods:title),' ')"/><xsl:text>)</xsl:text>
			</xsl:if>	
			<xsl:if test="../mods:relatedItem[@type='constituent']">
				<xsl:text> - </xsl:text><xsl:value-of select="string-join(../descendant::mods:relatedItem/mods:titleInfo/mods:title,' ')"/>
			</xsl:if>
		</dc:title>
	</xsl:template>
...
-->
	...
	<xsl:template match="mods:originInfo">
		...
		<xsl:if test="//mods:physicalLocation[@type='repository']">
			<dc:publisher>
				<xsl:text>repos:</xsl:text><xsl:value-of select="..//mods:physicalLocation[@type='repository']"/>
			</dc:publisher>
	    </xsl:if>
...
-->
<!--
	<xsl:template match="mods:location[not(mods:url/@displayLabel='thumb') and not(mods:url/@displayLabel='Thumbnail')]">
		<xsl:apply-templates select="mods:url"/>
        	<dc:relation><xsl:text>linkto:</xsl:text><xsl:value-of select="mods:url[not(@displayLabel='thumb')]"/></dc:relation>
	</xsl:template>
-->
<!--
	<xsl:template match="mods:location[mods:url/@displayLabel='thumb']">
        	<dc:relation><xsl:text>thumb:</xsl:text><xsl:value-of select="mods:url[@displayLabel='thumb']"/></dc:relation>
        	<dc:relation><xsl:text>full:</xsl:text><xsl:value-of select="substring-before(mods:url[@displayLabel='thumb'],'?')"/></dc:relation>
        	<dc:relation><xsl:text>linkto:</xsl:text><xsl:value-of select="mods:url[not(@displayLabel='thumb')]"/></dc:relation>
	</xsl:template>
-->
	...
	<xsl:template match="mods:relatedItem[not(//mods:recordIdentifier/@source='MH:OASIS')][mods:titleInfo | mods:name | mods:identifier | mods:location[not(mods:url)]]">
		<xsl:choose>
...
<xsl:template match="mods:relatedItem" mode="componenthierarchy">
	...
	<xsl:template match="mods:relatedItem[@type='constituent']">
		<xsl:apply-templates select="mods:location[mods:url/@access='preview']"/>
		<xsl:apply-templates select="mods:location[not(mods:url/@access='preview') and mods:url/@access='raw object']"/>
	</xsl:template>

<!-- Harvard 2015-05-23 use this template instead to increment thumb/full pairs  - multiple images from via possible -->
	<xsl:template match="mods:location[mods:url/@access='preview']">
		<xsl:variable name="increment"><xsl:value-of select="count(preceding-sibling::*[mods:url/@access='preview']) + 1"/></xsl:variable>
        <dc:relation><xsl:text>thumb:</xsl:text><xsl:value-of select="format-number($increment,'000')"/><xsl:text>:</xsl:text><xsl:value-of select="mods:url[@access='preview']"/></dc:relation>
<xsl:choose>
    <xsl:when test="contains(mods:url[@access='preview'],'?')">
        <dc:relation><xsl:text>full:</xsl:text><xsl:value-of select="format-number($increment,'000')"/><xsl:text>:</xsl:text><xsl:value-of select="substring-before(mods:url[@access='preview'],'?')"/></dc:relation>
    </xsl:when>
    <xsl:otherwise>
       	<dc:relation><xsl:text>full:</xsl:text><xsl:value-of select="format-number($increment,'000')"/><xsl:text>:</xsl:text><xsl:value-of select="mods:url[@access='raw object']"/></dc:relation>
    </xsl:otherwise>
</xsl:choose>
       	<dc:relation><xsl:text>linkto:</xsl:text><xsl:value-of select="format-number($increment,'000')"/><xsl:text>:</xsl:text><xsl:value-of select="mods:url[@access='raw object']"/></dc:relation>
	</xsl:template>

	<xsl:template match="mods:location[not(mods:url/@access='preview') and mods:url/@access='raw object']">
		<xsl:variable name="increment"><xsl:value-of select="count(preceding-sibling::*[mods:url/@access='raw object']) + 1"/></xsl:variable>
       	<dc:relation><xsl:text>linkto:</xsl:text><xsl:value-of select="format-number($increment,'000')"/><xsl:text>:</xsl:text><xsl:value-of select="mods:url[@access='raw object']"/></dc:relation>
	</xsl:template>

<!--
    <xsl:template match="mods:url[@displayLabel='Thumbnail']">
	 <dc:relation><xsl:text>thumb:</xsl:text><xsl:value-of select="."/></dc:relation>
    </xsl:template>

    <xsl:template match="mods:url[@displayLabel='Full Image']">
	 <dc:relation><xsl:text>full:</xsl:text><xsl:value-of select="."/></dc:relation>
    </xsl:template>
-->
